...
function GroupsMode({search, navigation}) {
  const {authAxios} = useContext(AxiosContext);
  let [loading, setLoading] = useState(true);
  let [groupsFinds, setGroupsFinds] = useState([]);
        ...
        ListEmptyComponent={() => (
          <View style={styles.noFound}>
            <Text style={styles.noFoundText}>
              Lo sentimos no pudimos encontrar a ningún grupo que contenga en su
              nombre "<Text style={styles.noFoundSearch}>{search}</Text>"
            </Text>
          </View>
        )}
      />
